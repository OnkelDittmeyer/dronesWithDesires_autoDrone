...
// ---------------------- SETTINGS--------------------------------------------------------
//Settings for automated drone system "Drones with Desires"

//movement duration (in milliseconds)
int flapMin = 1500;      //min 1,5sec
int flapMax = 5000;      //max 5sec

//delay in between random flaps (higher = more time of nothing happening)
int delayMin = 10000;    // min 10sec
int delayMax = 60000;    // max 60sec

// additional sensor interaction
...
void loop()
{
  ...
  for (int i = 0; i < motorNum; i++) {
    motors[i].update();
    if (motors[i].onOff && motors[i].idleMode){
      Serial.print("[x] "); 
      idleMotors++;
    }
    if (!motors[i].onOff && motors[i].idleMode){
    ...
    if (!motors[i].onOff && !motors[i].idleMode || motors[i].onOff && !motors[i].idleMode){
      Serial.print("[ ] ");
    }
  }
  Serial.println();

  //trigger Sensors and start up to three motors
  if(sensorActive){
    idleSensors();
  }

  //start idle motors if available slots
  if(idleMotors < idleMax){
      motors[random(0,6)].idle(random(flapMin,flapMax),random(delayMin,delayMax));
  }
